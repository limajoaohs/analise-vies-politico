...
def gerar_base_politica_internacional():
    print("Gerando Dataset Internacional Político (20Newsgroups Politics)...")
    ...
    df_balanced = pd.concat([
        df_0.sample(n=min_amostras, random_state=42, replace=True),
        df_1.sample(n=min_amostras, random_state=42, replace=True),
    ]).sample(frac=1, random_state=42).reset_index(drop=True)

    if len(df_balanced) < 10000:
        multiplicador = (10000 // len(df_balanced)) + 1
        df_balanced = pd.concat([df_balanced] * multiplicador, ignore_index=True).head(10000)
