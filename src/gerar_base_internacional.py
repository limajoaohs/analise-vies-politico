...
def gerar_base_politica_internacional():
    print("Gerando Dataset Internacional Político (20Newsgroups)...")
    categorias_politicas = ["talk.politics.guns", "talk.politics.mideast", "talk.politics.misc"]
    
    news_all = fetch_20newsgroups(subset="all", categories=categorias_politicas, remove=("headers", "footers", "quotes"))
    
    df = pd.DataFrame({"texto": news_all.data, "target_orig": news_all.target})
