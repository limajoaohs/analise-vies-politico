...
def treinar_e_avaliar_bow(caminho_csv, nome_base='Dataset'):
    ...
    if not coluna_label:
        print(f'Erro: Nenhuma coluna de rótulo encontrada.')
        return

    mapa_conversao = {
        'esquerda': 0, 'left': 0, '0': 0, 0: 0,
        'direita': 1, 'right': 1, '1': 1, 1: 1,
    }

    df['label_num'] = df[coluna_label].astype(str).str.lower().str.strip().map(mapa_conversao)

    df = df.dropna(subset=[coluna_texto, 'label_num'])
    df = df[df[coluna_texto].astype(str).str.strip() != '']
...
if __name__ == '__main__':
    ...
    if os.path.exists(caminho_inter):
        treinar_e_avaliar_bow(caminho_inter_limpo, nome_base='Base Internacional')
    else:
        print(f"Base internacional não encontrada.")

    caminho_br = 'data/dataset_brasil.csv'
    caminho_br_limpo = 'data/dataset_brasil_limpo.csv'

    if os.path.exists(caminho_br):
        treinar_e_avaliar_bow(caminho_br_limpo, nome_base='Base Brasileira (PT-BR)')
    else:
        print(f"Base brasileira não encontrada.")