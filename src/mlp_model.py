...
def run_mlp_pipeline(data_path, lang_label, save_prefix):
    ...
    plt.title(f'Matriz de Confusão - MLP ({lang_label})')
    plt.ylabel('Real')
    plt.xlabel('Previsto')
    plt.tight_layout()
    
    os.makedirs('results', exist_ok=True)
    plt.savefig(f'results/cm_mlp_{save_prefix}.png', dpi=300)
    plt.close()

if __name__ == '__main__':
    run_mlp_pipeline('data/dataset_internacional_limpo.csv', 'Internacional', 'intl')
    run_mlp_pipeline('data/dataset_brasil_limpo.csv', 'Brasil', 'br')