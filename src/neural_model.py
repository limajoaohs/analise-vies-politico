...
def treinar_e_avaliar_neural(caminho_csv, nome_base='Dataset'):
    ...
    train_loader = DataLoader(
        DatasetTexto(X_train, y_train), batch_size=32, shuffle=True
    )
    test_loader = DataLoader(
        DatasetTexto(X_test, y_test), batch_size=32, shuffle=False
    )

    model = ClassificadorBiLSTM(vocab_size=len(vocab.word2idx)).to(device)
